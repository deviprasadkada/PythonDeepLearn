...
df = pd.read_csv('D:\Downloads\DeepLearning_Lesson2\DeepLearning_Lesson2\\kc_house_data.csv')
...
def basic_model_1(x_size, y_size):
    ...
    t_model.add(Dense(80, activation="sigmoid"))
    t_model.add(Dense(50, activation="relu"))
    t_model.add(Dense(y_size))
    print(t_model.summary())
    t_model.compile(loss='mean_squared_error',
        optimizer=Adam(), # update and tune the Model's parameters in a direction so that we can minimize the Loss function.
        metrics=[metrics.mae])
    return(t_model)
